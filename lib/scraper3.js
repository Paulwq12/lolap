...
async function xnxxSearch(query) {
  ...
  const response = await fetch(
    `${baseUrl}/search/${query}/0`,  // Always fetch the first page
    { method: 'get' }
  );
  ...
  // Extract video details: title, link, thumbnail, and duration
  $('div.mozaique > div').each(function (index, element) {
    if (count >= 30) return false;  // Stop after 30 results

    // Extract title and link
    const title = $(element).find('a').attr('title') || 
                  $(element).find('.thumb-under > p').text().trim() || 
                  "Untitled"; // Additional fallback if title attribute is missing
    const link = baseUrl + $(element).find('a').attr('href');

    // Extract high-quality thumbnail URL
    const thumb =
      $(element).find('a > img').attr('data-thumb') ||   // High-quality thumbnail, if available
      $(element).find('a > img').attr('data-src') ||     // Fallback to lazy-loaded image
      $(element).find('a > img').attr('src') ||          // Fallback to lower-quality image
      "No Thumbnail";

    // Extract duration
    const duration = $(element).find('.thumb-under > p.metadata > span.duration').text().trim() || 
                     "No Duration"; // Adjusted selector for duration
    ...
    count++;  // Increment the count after each video is added
  });

  return results; // Return the first 30 search results
}
  ...
  return new Promise(async _0x610d9 => {
    ...
    _0x585cba('div.mozaique\x20>\x20div\x20>\x20div.thumb-under\x20>\x20p.title')[_0x524197(0x145)](
      function (_0x12af35, _0x1d32eb) {
        ...
        if (_0x275f00.length < 20) {  // Limit to the first 30
    ...
    _0x585cba('div.mozaique\x20>\x20div\x20>\x20div.thumb-under')['each'](
      function (_0x10ee54, _0x1189d2) {
        ...
        if (_0x6d294b.length < 20) {  // Limit to the first 30
    ...
    _0x585cba(_0x524197(0x139))[_0x524197(0x145)](function (_0x1f8b18, _0x5a6c0d) {
      ...
      if (_0x518839.length < 30) {  // Limit to the first 30
        _0x518839['push'](
    ...
    // Fetch video sizes and add them to the search results
    for (let _0x3e4544 = 0x0; _0x3e4544 < Math.min(30, _0x275f00.length); _0x3e4544++) {
      try {
        const videoUrl = _0x51afd2[_0x3e4544];
        const videoResponse = await fetch(videoUrl);
        const buffer = await videoResponse.arrayBuffer();
        const fileSizeInMB = (buffer.byteLength / (1024 * 1024)).toFixed(2); // Convert bytes to MB
        ...
        _0x5d18a0.push({
          ...
          url: videoUrl,
          size: `${fileSizeInMB} MB`  // Add file size in MB
        });
      } catch (fetchError) {
        console.error(`Error fetching video size for ${_0x51afd2[_0x3e4544]}:`, fetchError);
        _0x5d18a0.push({
          title: _0x275f00[_0x3e4544],
          duration: _0x437564[_0x3e4544],
          quality: _0x6d294b[_0x3e4544],
          ...
          size: 'Unknown'  // Default value in case of an error
        });
      }
    }

    _0x610d9(_0x5d18a0);  // Return the search results
  });
}
