...
exports.ssweb = (url) => {
    return new Promise((resolve, reject) => {
        ...
        fetcher(fullUrl, {
            method: 'GET',
            headers: {} // No body here
        })
        .then(async response => {
            if (!response.ok) {
                return reject({
                    status: response.status,
                    message: `Error fetching screenshot: ${response.statusText}`
                });
            }
            ...
            resolve({
                status: 200,
                result: buffer,
                websiteUrl: url, // Include the original URL in the resolved object
            });
        })
        .catch(error => {
            console.error('[DEBUG] Fetch encountered an error:', error.message);
            reject({
                status: 500,
