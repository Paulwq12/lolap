...
exports.totalcase = () => {
   var file = fs.readFileSync("../XeonCheems14.js").toString()
...
exports.smsg = (XeonBotInc, m, store) => {
  ...
  // Check if the message is deleted and handle it
  if (m.message && m.message.protocolMessage) {
    const protocolMessage = m.message.protocolMessage;
    if (protocolMessage.type === 2) { // Protocol message type 2 indicates a deleted message
      m.deleted = true;
  ...
  if (m.message) {
    ...
    m.body = m.message.conversation || m.msg.caption || m.msg.selectedId || m.msg.text || 
